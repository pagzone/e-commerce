...
const Header = () => {
  ...
  const logo = useMemo(
    () => (
      <Image
        src="/pagzone-ecommerce-logo.png"
        width={50}
        height={50}
        alt="Logo"
      />
      ...
      <div className="flex items-center gap-3.5">
        {authIcons.map((value, i) => (
          <Link href={value.href} key={i} className="relative">
            ...
            {i < 2 && (
              <span className="absolute top-0 right-0 bg-blue-500 rounded-full w-3.5 h-3.5 text-[11px] text-center">
                1
              </span>
            )}
          </Link>
  ...
  return (
    <header className="flex flex-col h-16 md:h-28 px-4">
      ...
      <div className="flex justify-between items-center py-6 px-1 md:hidden">
        ...
        <div className="flex items-center gap-x-8">
          ...
          <Link href="/" className="relative">
            <ShoppingBag size={27} />
            <span className="absolute top-0 right-0 bg-blue-500 rounded-full w-3.5 h-3.5 text-white text-[11px]">
              0
            </span>
          </Link>
        </div>
      ...
      {/* Desktop Header */}
      <div className="hidden md:flex items-center justify-between mx-2 md:mx-14 lg:mx-24 xl:mx-44 py-4 border-b">
      ...
      <nav className="hidden md:flex flex-col gap-2 mt-2 mx-14 lg:mx-24 xl:mx-44">
        <ProductMenu />
      </nav>
    </header>
  );
